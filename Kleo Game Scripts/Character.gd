...
#STAMINA
	...
	if direction:
		if is_on_floor():
			...
			if Input.is_action_pressed("sprint") && STAMINA > 0:
				velocity.x = lerp(velocity.x, direction.x * (MOVE_SPEED * (13 / 4)) * SPRINT_SPEED, delta * 0.75)
				velocity.z = lerp(velocity.z, direction.z * (MOVE_SPEED * (13 / 4)) * SPRINT_SPEED, delta * 0.75)
